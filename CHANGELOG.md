...
- **Versioning scheme: calendar (`YEAR.MAJOR.MINOR`) → SemVer.** All `2026.x`
  releases of `tuvl` and `tuvl-insight` are yanked on PyPI (pre-1.0 internal
  line). Pinned installs keep resolving with a warning; unpinned installs land
  on `1.0.0`. A project pinning `tuvl>=2026.*` should change the floor to
  `tuvl>=1.0.0`.
